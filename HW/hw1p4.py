	...
	for i in range(11):
		for j in range(11-i):
			for k in range(11-i-j):
				...
				vol, daily_ret, sharpe, cum_ret = simulate_internal(na_normalized_price, alloc, days)
...
def main():
	...
	# read prices and normalize
	na_price = read_closing(dt_start, dt_end, ls_symbols)[0]
	na_normalized_price = na_price / na_price[0, :]

	# compute optimized portfolio
	...
	# compute portfolio normalized prices
	na_normalized_price_port = np.dot(na_normalized_price, alloc)
	na_normalized_price_port = np.reshape(na_normalized_price_port, (len(na_normalized_price_port), 1))

	# appends portfolio as a new equity
	na_normalized_price = np.append(na_normalized_price, na_normalized_price_port, axis=1)
	ls_symbols.insert(len(ls_symbols), 'portfolio')
	...
	plt.xlabel('Date')
	#plt.savefig('hw1p4.pdf', format='pdf')
	plt.show()
